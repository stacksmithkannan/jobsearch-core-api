    ...
    public class ApplyToJobHandler : IRequestHandler<ApplyToJobCommand, string>
    {
        ...
        public async Task<string> Handle(ApplyToJobCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (user == null)
                {
                    _logger.LogWarning("Job application failed: User not found for email {Email}", email);
            ...
            {
                _logger.LogError(ex, "An unexpected error occurred while applying for job ID {JobId}", request.JobId);
                return "An error occurred while processing your application. Please try again later.";
            }
        }
    }
}
